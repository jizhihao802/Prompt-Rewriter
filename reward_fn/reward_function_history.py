...
def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    ...
    tokens = _tokenizer.encode(original_instruction, add_special_tokens=False)
    num_tokens_instruction = len(tokens)#统计原指令的tokens数目作为修改模型生成修改后指令的max_new_tokens

    # 获取原始相似度、baseline相似度和对话历史
    ...
    dialog_history = ""
    if extra_info and "dialog_history" in extra_info:
        dialog_history = extra_info["history"].strip()
    ...
    # 3. 生成优化后指令i
    modify_prompt = (
        f"你是一个指令优化器，负责优化用户给到大模型的指令，请结合<history>和</history>之间的对话历史在保留信息完整的前提下按照{modify_methods}的方法优化<instruction>和</instruction>之间的指令。"
        f"<history>\n{dialog_history}\n</history>\n"
        f"<instruction>\n{original_instruction}\n</instruction>"
    ...
    try:
        modified_instruction = generate_local_response(modify_prompt, num_tokens_instruction)
    except Exception as e:
    ...
    if dialog_history:
        full_prompt = f"{dialog_history}\n<user>:{rewrited_prompt}\n<assistant>:"
    else:
        full_prompt = f"<user>:{rewrited_prompt}\n<assistant>:"

    try:
        response_new = generate_local_response(full_prompt, num_tokens_truth)
    except Exception as e:
