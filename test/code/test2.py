...
def evaluate_parquet_test(test_file, llm, llm_agent, model_sbert, tokenizer):
    ...
    输出三个均值：
        ...
        3. original
    """
    log_path = "/root/autodl-tmp/test/log.txt"
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        try:
            ...
            if "4" in methods:
                ...
                modified_instruction = extract_instruction(raw_modified)

                new_answer = generate_local_response(llm, modified_instruction, sampling_llm, num_tokens_truth)
...
if __name__ == "__main__":
    ...
    # 测试文件路径
    test_file_path = "/root/autodl-tmp/data/processed_sharegpt/test/test2_shuffle.parquet"
